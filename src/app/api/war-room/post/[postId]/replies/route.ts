...
export async function GET(
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ replies: [], total: 0, flagged: 0, questions: 0, too_old: false }, { status: res.status });
    return NextResponse.json(await res.json());
  } catch {
    return NextResponse.json({ replies: [], total: 0, flagged: 0, questions: 0, too_old: false }, { status: 502 });
  }
}
